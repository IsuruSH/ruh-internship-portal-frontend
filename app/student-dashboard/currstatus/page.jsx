...
export default function CurrentStatus() {
    ...
    const loadStatus = async () => {
      try {
        ...
        if (data.statusTimeline && data.statusTimeline.length > 0) {
          setTimelineData(data.statusTimeline);
  ...
  const getStepDate = (stepId) => {
    ...
    if (!statusKey) return steps[stepId - 1].defaultDate;

    const timelineEntry = timelineData.find(
      (item) => item.status === statusKey
    );

    return timelineEntry
      ? new Date(timelineEntry.date_achieved).toLocaleDateString("en-US", {
          year: "numeric",
          month: "short",
          day: "numeric",
        })
      : steps[stepId - 1].defaultDate;
  };
  ...
  const updateStatus = async (stepId) => {
    if (stepId <= activeStep + 1) {
      try {
        ...
        const statusKey = Object.keys(statusToStepId).find(
          (key) => statusToStepId[key] === stepId - 1
        );
        ...
        if (statusKey) {
          ...
          // Refresh timeline data after update
          const updatedData = await fetchStatusTimeline(user.id);
          setTimelineData(updatedData.statusTimeline);
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{stepDate}</p>
                      </div>
                    </div>
                    <FaChevronDown
                    ...
                    <div className="mt-4 pl-10 space-y-4">
                      ...
                      {step.id === activeStep && step.id < steps.length && (
                        <div className="pt-4 border-t border-gray-100">
                          <button
                            onClick={() => {
                              updateStatus(step.id + 1);
                              step.id === 5 &&
                                router.push("/student-dashboard/about");
                              step.id === 5 && window.location.reload();
                            }}
                            className="px-4 py-2 bg-[#0F1D2F] text-white rounded-md text-sm hover:bg-[#1E3A8A]"
                          >
                            Mark as {steps[step.id - 1].title}
                          </button>
                        </div>
                      )}
